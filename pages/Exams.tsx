...
export default function Exams() {
  ...
  const selectedExam = useMemo(() => exams.find((exam) => exam.id === roomManager.examId) || null, [exams, roomManager.examId]);
  const selectedExamAssignments = useMemo(() => selectedExam ? (assignmentsByExamId.get(selectedExam.id) || []) : [], [assignmentsByExamId, selectedExam]);
          ...
          <div style={modalCard} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 12 }}>
              <div>
                ...
                <div style={{ opacity: 0.85 }}>{selectedExam.subject} — {selectedExam.dateISO} — {selectedExam.period}</div>
              </div>
              <div style={{ fontWeight: 900, color: roomManager.selectedRoomIds.length === selectedExam.roomsCount ? "#22c55e" : "#f59e0b" }}>
                  ...
                  <table style={{ width: "100%", minWidth: 860 }}>
                    ...
                    <tbody>
                      {selectedExamAvailableRooms.map((room) => {
                        ...
                        const disabled = room.blocked || room.inactive || (!checked && roomManager.selectedRoomIds.length >= selectedExam.roomsCount);
                        return (
                          <tr key={room.id}>
                            <td style={tdStyle}>
                              <input type="checkbox" checked={checked} disabled={disabled} onChange={() => toggleRoomSelection(room.id)} />
                            </td>
                            ...
                            <td style={tdStyle}>
                              {room.blocked ? "محظورة" : room.inactive ? "موقوفة" : "متاحة"}
                            </td>
                            <td style={tdStyle}>
                              {room.blocked ? "يوجد حظر في نفس التاريخ/الفترة" : room.inactive ? "القاعة غير نشطة" : "يمكن ربطها"}
                            </td>
                          </tr>
                        );
                      })}
                    ...
                    <tr key={e.id} className={e.dateISO === todayISO ? "row-today" : undefined}>
                      ...
                      <td style={tdStyle}>
                        {(() => {
                          ...
                          const blockedAssigned = assigned.filter((row) => isRoomBlockedForExam(row.roomId, e, activeBlocks)).length;
                          const complete = assigned.length === e.roomsCount && blockedAssigned === 0;
                          return (
                            <button
                              style={{
                                ...btn(complete ? "#10b981" : assigned.length === 0 ? "#ef4444" : "#f59e0b", "#07101f"),
                                padding: "8px 12px",
                              }}
