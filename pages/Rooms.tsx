...
function toCSV(rows: Room[]) {
  ...
  const lines = [
    header.join(","),
    ...rows.map((r) => [r.roomName, r.code || "", r.building, r.type, String(r.capacity), r.status || "active", r.notes].map(escape).join(",")),
  ];
  return lines.join("\n");
}
...
export default function Rooms() {
  ...
  const normalizedBlocks = useMemo(() => roomBlocks.map((block) => ({
    ...block,
    status: block.status === "cancelled" ? "cancelled" : block.endDate < todayISO ? "expired" : "active",
  })), [roomBlocks, todayISO]);

  const blockedRoomIdsToday = useMemo(() => new Set(
    rooms
  ...
  const filtered = useMemo(() => {
    ...
    return rooms.filter((r) => {
      const matchesQuery = !q || [r.roomName, r.code || "", r.building, r.type, String(r.capacity), r.notes].some((x) => String(x).includes(q));
  ...
  const historyBlocks = useMemo(() => {
    ...
    return normalizedBlocks.filter((block) => block.roomId === historyRoomId).sort((a, b) => b.startDate.localeCompare(a.startDate));
  ...
  function validate(r: Room) {
    ...
    if (duplicateCode) {
      const exists = rooms.some((room) => String(room.code || "").trim() === duplicateCode && room.id !== r.id);
  ...
  function saveAdd() {
    ...
    setRooms((prev) => [{ ...row, status: row.status || "active" }, ...prev]);
    setAdding(false);
    setRow({ ...emptyRoom, id: createId("room") });
  }
  ...
  function saveEdit() {
    ...
    if (msg) return alert(msg);
    setRooms((prev) => prev.map((x) => (x.id === editingId ? { ...edit, status: edit.status || "active" } : x)));
    setEditingId(null);
    setEdit({ ...emptyRoom });
  }
  ...
  const setCurrent = (patch: Partial<Room>) => {
    if (editingId) setEdit((prev) => ({ ...prev, ...patch }));
    else setRow((prev) => ({ ...prev, ...patch }));
  };
  ...
  function saveQuickBlock() {
    ...
    const overlap = roomBlocks.some((block) =>
      block.roomId === quickBlock.roomId &&
      block.status === "active" &&
      !(quickBlock.endDate < block.startDate || quickBlock.startDate > block.endDate) &&
      (block.session === "full-day" || quickBlock.session === "full-day" || block.session === quickBlock.session)
    );
    ...
    <div style={pageStyle} ref={topRef}>
          ...
          <div style={modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(2, minmax(220px, 1fr))" }}>
              <div>
                ...
                <textarea style={{ ...inputStyle, minHeight: 90 }} value={quickBlock.reason} onChange={(e) => setQuickBlock((prev) => ({ ...prev, reason: e.target.value }))} />
              </div>
              <div>
                <div style={{ fontWeight: 900, marginBottom: 6 }}>نوع السبب</div>
                <GoldDropdown value={quickBlock.reasonType} options={BLOCK_REASON_OPTIONS} onChange={(v) => setQuickBlock((prev) => ({ ...prev, reasonType: v }))} />
                <div style={{ fontWeight: 900, marginBottom: 6, marginTop: 10 }}>الفترة</div>
                <GoldDropdown value={quickBlock.session} options={BLOCK_SESSION_OPTIONS} onChange={(v) => setQuickBlock((prev) => ({ ...prev, session: v as QuickBlockState["session"] }))} />
              </div>
              <div>
                <div style={{ fontWeight: 900, marginBottom: 6 }}>تاريخ البداية</div>
                <input style={inputStyle} type="date" value={quickBlock.startDate} onChange={(e) => setQuickBlock((prev) => ({ ...prev, startDate: e.target.value }))} />
              </div>
              <div>
                <div style={{ fontWeight: 900, marginBottom: 6 }}>تاريخ النهاية</div>
                <input style={inputStyle} type="date" value={quickBlock.endDate} onChange={(e) => setQuickBlock((prev) => ({ ...prev, endDate: e.target.value }))} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              <button style={btn("#10b981", "#07101f")} onClick={saveQuickBlock}>حفظ الحظر</button>
              <button style={btn("#1f2937", "#d4af37")} onClick={() => setQuickBlock((prev) => ({ ...prev, open: false }))}>إلغاء</button>
            </div>
          </div>
        </div>
      )}
      ...
      {historyRoomId && (
        <div style={modalOverlay} onClick={() => setHistoryRoomId(null)}>
          <div style={modalCard} onClick={(e) => e.stopPropagation()}>
              ...
              <table style={{ width: "100%", minWidth: 720 }}>
                ...
                <tbody>
                  {historyBlocks.length === 0 ? (
                    <tr><td style={tdStyle} colSpan={6}>لا يوجد سجل حظر لهذه القاعة.</td></tr>
                    ...
                    <tr key={block.id}>
                      ...
                      <td style={tdStyle}>{block.status === "active" ? "نشط" : block.status === "expired" ? "منتهي" : "ملغي"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              <button style={btn("#1f2937", "#d4af37")} onClick={() => setHistoryRoomId(null)}>إغلاق</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={header}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <button style={btn("#1f2937", "#d4af37")} onClick={() => history.back()}>← رجوع</button>
          <button style={btn("#3b82f6", "#07101f")} onClick={startAdd}>+ إضافة قاعة</button>
          <button style={btn("#ef4444", "#07101f")} onClick={deleteAll}>🗑 حذف الكل</button>
        </div>
      </div>
      ...
      <div style={card}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <input style={{ ...inputStyle, maxWidth: 340 }} placeholder="بحث..." value={query} onChange={(e) => setQuery(e.target.value)} />
          <GoldDropdown
            value={statusFilter}
            options={[
          ...
          />
          <button style={btn("#22c55e", "#07101f")} onClick={exportCSV}>تصدير CSV</button>
          <button style={btn("#10b981", "#07101f")} onClick={exportExcel}>تصدير Excel</button>
        ...
        <div style={card}>
          <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(4, minmax(220px, 1fr))" }}>
            ...
            <div>
              ...
              <input style={inputStyle} value={current.code || ""} onChange={(e) => setCurrent({ code: e.target.value })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>المبنى</div>
              <GoldDropdown value={current.building} options={BUILDING_OPTIONS} placeholder="— اختر المبنى —" onChange={(v) => setCurrent({ building: v })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>نوع القاعة</div>
              <GoldDropdown value={current.type} options={ROOM_TYPE_OPTIONS} placeholder="— اختر النوع —" onChange={(v) => setCurrent({ type: v })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>السعة</div>
              <input style={inputStyle} type="number" value={String(current.capacity)} onChange={(e) => setCurrent({ capacity: Number(e.target.value) || 0 })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>الحالة</div>
              <GoldDropdown value={current.status || "active"} options={ROOM_STATUS_OPTIONS} onChange={(v) => setCurrent({ status: v as Room["status"] })} />
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>ملاحظات</div>
              <textarea style={{ ...inputStyle, minHeight: 80 }} value={current.notes} onChange={(e) => setCurrent({ notes: e.target.value })} />
            </div>
          </div>
        ...
        <div style={tableWrap}>
          <table style={{ width: "100%", minWidth: 1280 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td style={tdStyle} colSpan={9}>لا توجد بيانات.</td>
                </tr>
              ) : filtered.map((r) => {
                  ...
                  <tr key={r.id}>
                    ...
                    <td style={tdStyle}>
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <button style={btn("#f59e0b", "#07101f")} onClick={() => openQuickBlock(r)}>⛔ حظر</button>
                        <button style={btn("#334155", "#e6c76a")} onClick={() => setHistoryRoomId(r.id)}>📜 السجل</button>
