...
export default function RoomBlocks() {
  ...
  const roomsOptions = useMemo(
    () => [{ value: "", label: "— اختر القاعة —" }, ...rooms.map((room) => ({ value: room.id, label: room.roomName }))],
    [rooms]
  );

  const normalizedBlocks = useMemo(
    () => roomBlocks.map((block) => ({ ...block, status: blockStatusLabel(block, todayISO) })),
    [roomBlocks, todayISO]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return normalizedBlocks
      .filter((block) => {
        const matchesQuery = !q || [block.roomName, block.reason, block.reasonType, block.startDate, block.endDate, block.session].some((x) => String(x).includes(q));
        const matchesStatus = statusFilter === "all" || block.status === statusFilter;
        return matchesQuery && matchesStatus;
      })
      .sort((a, b) => b.startDate.localeCompare(a.startDate));
  }, [normalizedBlocks, query, statusFilter]);
  ...
  function validate(block: RoomBlock) {
    ...
    const overlap = roomBlocks.some((existing) =>
      existing.id !== block.id &&
      existing.roomId === block.roomId &&
      existing.status === "active" &&
      !(block.endDate < existing.startDate || block.startDate > existing.endDate) &&
      (existing.session === "full-day" || block.session === "full-day" || existing.session === block.session)
    );
  ...
  function saveAdd() {
    const prepared = { ...hydrateBlock(row), createdBy: String(user?.email || "").trim() || row.createdBy };
    const msg = validate(prepared);
    if (msg) return alert(msg);
    setRoomBlocks((prev) => [prepared, ...prev]);
    setAdding(false);
    setRow({ ...emptyBlock, id: createId("block") });
  }
  ...
  function saveEdit() {
    if (!editingId) return;
    const prepared = { ...hydrateBlock(edit), createdBy: edit.createdBy || String(user?.email || "").trim() || undefined };
    const msg = validate(prepared);
    if (msg) return alert(msg);
    setRoomBlocks((prev) => prev.map((block) => (block.id === editingId ? prepared : block)));
    setEditingId(null);
    setEdit({ ...emptyBlock });
  }
  ...
  function cancelBlock(id: string) {
    ...
    setRoomBlocks((prev) => prev.map((block) => (block.id === id ? { ...block, status: "cancelled" } : block)));
  }

  function removeBlock(id: string) {
    if (!confirm("هل تريد حذف هذا السجل؟")) return;
    setRoomBlocks((prev) => prev.filter((block) => block.id !== id));
  }
    ...
    <div style={pageStyle} ref={topRef}>
      <div style={header}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button style={btn("#ef4444", "#07101f")} onClick={startAdd}>+ إضافة حظر</button>
        </div>
      </div>
      ...
      <div style={card}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <GoldDropdown value={statusFilter} options={STATUS_FILTER_OPTIONS} onChange={(v) => setStatusFilter(v as typeof statusFilter)} />
        ...
        <div style={card}>
          <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(4, minmax(220px, 1fr))" }}>
            <div>
              ...
              <GoldDropdown value={current.roomId} options={roomsOptions} onChange={(v) => {
                const room = rooms.find((item) => item.id === v);
                setCurrent({ roomId: v, roomName: room?.roomName || "" });
              }} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>نوع السبب</div>
              <GoldDropdown value={current.reasonType} options={REASON_OPTIONS} onChange={(v) => setCurrent({ reasonType: v })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>الفترة</div>
              <GoldDropdown value={current.session} options={SESSION_OPTIONS} onChange={(v) => setCurrent({ session: v as RoomBlock["session"] })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>الحالة</div>
              <GoldDropdown value={current.status} options={[{ value: "active", label: "نشط" }, { value: "cancelled", label: "ملغي" }]} onChange={(v) => setCurrent({ status: v as RoomBlock["status"] })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>تاريخ البداية</div>
              <input style={inputStyle} type="date" value={current.startDate} onChange={(e) => setCurrent({ startDate: e.target.value })} />
            </div>
            <div>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>تاريخ النهاية</div>
              <input style={inputStyle} type="date" value={current.endDate} onChange={(e) => setCurrent({ endDate: e.target.value })} />
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
              <div style={{ fontWeight: 900, marginBottom: 6, color: "#d4af37" }}>سبب الحظر</div>
              <textarea style={{ ...inputStyle, minHeight: 90 }} value={current.reason} onChange={(e) => setCurrent({ reason: e.target.value })} />
            </div>
          </div>
      ...
      <div style={card}>
        <div style={tableWrap}>
          <table style={{ width: "100%", minWidth: 1120 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td style={tdStyle} colSpan={9}>لا توجد بيانات.</td></tr>
              ) : filtered.map((block) => (
                <tr key={block.id}>
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button style={btn("#60a5fa", "#07101f")} onClick={() => startEdit(block)}>✏️ تعديل</button>
                      {block.status === "active" && <button style={btn("#f59e0b", "#07101f")} onClick={() => cancelBlock(block.id)}>إلغاء الحظر</button>}
                      <button style={btn("#ef4444", "#07101f")} onClick={() => removeBlock(block.id)}>🗑 حذف</button>
                    </div>
                  </td>
                </tr>
              ))}
