            ...
            return (
              <div key={item.label} style={{ display: "grid", gap: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  ...
                  <div style={{ color: item.color || GOLD, fontWeight: 900 }}>{item.value}{item.subLabel ? ` ${item.subLabel}` : ""}</div>
                </div>
                <div style={{ height: 16, borderRadius: 999, overflow: "hidden", background: "rgba(255,255,255,0.08)" }}>
                  <div style={{ height: "100%", width: `${width}%`, borderRadius: 999, background: item.color || GOLD }} />
                </div>
              </div>
            );
          })}
...
function InsightCard({ item, lang }: { item: Insight; lang: "ar" | "en" }) {
...
async function loadAnalyticsModel(tenantId: string): Promise<AnalyticsModel> {
  const [teachers, exams, rooms, roomBlocks, examRoomAssignments] = await Promise.all([
    loadTenantArray<Teacher>(tenantId, "teachers").catch(() => []),
    loadTenantArray<Exam>(tenantId, "exams").catch(() => []),
    loadTenantArray<Room>(tenantId, "rooms").catch(() => []),
    loadTenantArray<RoomBlock>(tenantId, "roomBlocks").catch(() => []),
    loadTenantArray<ExamRoomAssignment>(tenantId, "examRoomAssignments").catch(() => []),
  ]);

  const run = loadRun(tenantId);
  const taskAssignments = Array.isArray(run?.assignments) ? run.assignments : [];

  return { teachers, exams, rooms, roomBlocks, examRoomAssignments, taskAssignments };
}
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      setLoading(true);
      const next = await loadAnalyticsModel(tenantId);
      if (!active) return;
      setModel(next);
      setLoading(false);
    }
  ...
  const derived = useMemo(() => {
    ...
    const examsSorted = [...model.exams].sort((a, b) => String(a.dateISO).localeCompare(String(b.dateISO)));
    const examsByDay = new Map<string, number>();
    const examsBySubject = new Map<string, number>();
    const roomDemandByDay = new Map<string, number>();
    const blockedAssignments: { exam: string; room: string }[] = [];
    const unfilledExams: Exam[] = [];

    examsSorted.forEach((exam) => {
      const date = String(exam.dateISO || "");
      examsByDay.set(date, (examsByDay.get(date) || 0) + 1);
      examsBySubject.set(String(exam.subject || "بدون مادة"), (examsBySubject.get(String(exam.subject || "بدون مادة")) || 0) + 1);
      roomDemandByDay.set(date, (roomDemandByDay.get(date) || 0) + Math.max(1, Number(exam.roomsCount) || 1));
  ...
  const roomHealth = useMemo(() => {
    ...
    const blockedRate = pct(blockedActive, Math.max(1, model.rooms.length));
    return {
      activeRooms,
      blockedActive,
      blockedRate,
  ...
  }, [derived.activeRooms.length, derived.activeBlocks.length, model.rooms.length]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ color: "#d1d5db", lineHeight: 1.8, maxWidth: 900 }}>
          {tr(
            "هذه الصفحة أصبحت تعرض صحة التشغيل الفعلية للنظام من بيانات المعلمين والامتحانات والقاعات والحظر وربط القاعات ونتائج التوزيع الحالية، مع ملاحظات ذكية قابلة للتنفيذ.",
            "This page now shows real operational health from teachers, exams, rooms, blocks, room assignments, and the current distribution run, with actionable smart insights."
          )}
        </div>
      </div>
