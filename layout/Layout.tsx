...
import { TENANT_NAV_ITEMS, tenantPath } from "../config/tenantRoutes";
...
export default function Layout() {
  ...
  const sidebarItems = useMemo(() => {
    return TENANT_NAV_ITEMS
      .map((it) => ({
        to: it.path.startsWith('/system') ? it.path : tenantPath(routeTenantId || null, it.path),
        label: lang === 'ar' ? it.ar : it.en,
        icon: it.icon,
        adminOnly: it.adminOnly,
        systemOnly: it.systemOnly,
        superOnly: it.superOnly,
      }))
  ...
  }, [isAdmin, canSeeOwnerTools, canSeeSystemArea, routeTenantId, lang]);
