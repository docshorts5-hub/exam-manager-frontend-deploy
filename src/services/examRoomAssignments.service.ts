...
export function subscribeExamRoomAssignments(tenantId: string, onChange: (items: ExamRoomAssignment[]) => void, onError?: (error: unknown) => void) {
  ...
  let active = true;
  loadExamRoomAssignments(tenantId)
    .then((items) => {
