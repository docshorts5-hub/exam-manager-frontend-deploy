...
export function subscribeRoomBlocks(
  ...
  const repo = roomBlocksRepository as typeof roomBlocksRepository & {
    subscribe?: (tenantId: string, onChange: (items: RoomBlock[]) => void, onError?: (error: unknown) => void) => (() => void) | void;
  };
