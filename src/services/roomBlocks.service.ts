...
export function subscribeRoomBlocks(tenantId: string, onChange: (items: RoomBlock[]) => void, onError?: (error: unknown) => void) {
  return roomBlocksRepository.subscribe(tenantId, onChange, onError);
}
