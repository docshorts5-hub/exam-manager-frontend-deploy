...
export function rebalanceFairDistribution(out: any, latestTeachers: any[], constraints: any) {
  ...
  if (swaps > 0) {
    ...
    out.warnings.unshift(tr(
      `✅ تم تحسين التوازن: إعادة توزيع ${swaps} مهمة لتحقيق عدالة أكبر.`,
      `✅ Balance improved: ${swaps} tasks were redistributed for better fairness.`
    ));
  }
...
export function rebalanceInvigilationsToEqualize(out: any, latestTeachers: any[], constraints: any) {
  ...
  if (moves > 0) {
    out.warnings.unshift(tr(
      `✅ تم تحسين مساواة المراقبة: تم نقل ${moves} مهمة مراقبة بين الكادر التعليمي.`,
      `✅ Invigilation equality improved: ${moves} invigilation tasks were moved across the teaching staff.`
    ));
  } else {
    out.warnings.unshift(tr(
      "⚠️ لم يمكن مساواة المراقبة بالكامل بسبب القيود/التعارضات الحالية.",
      "⚠️ Invigilation could not be fully equalized because of the current constraints/conflicts."
    ));
  }
