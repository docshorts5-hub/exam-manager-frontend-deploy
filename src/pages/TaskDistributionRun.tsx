  ...
  const readinessSnapshot = useMemo(() => {
    ...
    const forecastRows = forecastRowsBase
      .map((row: any) => {
        ...
        const existingKeys = new Set((Array.isArray(row.teacherSuggestions) ? row.teacherSuggestions : []).map((item: any) => `${String(item?.teacherId || "").trim()}__${String(item?.source || "").trim()}`));
        const mergedSuggestions = [
          ...(Array.isArray(row.teacherSuggestions) ? row.teacherSuggestions : []),
          ...transferSuggestions.filter((item: any) => !existingKeys.has(`${String(item?.teacherId || "").trim()}__${String(item?.source || "").trim()}`)),
        ].slice(0, 10);
