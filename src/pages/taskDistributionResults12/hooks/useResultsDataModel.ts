...
function normalizeStoredTaskType(rawTaskType: any): string {
  ...
  if (raw.includes("مراقب دور") || raw.includes("مراقب الدور") || raw.includes("مراجعة") || raw.includes("تصحيح")) return "DUTY_INVIGILATOR";
  return raw;
}

function normalizeResultsAssignmentSubject(taskType: string, assignment: any, normalizeSubject: (subject: string) => string) {
  const rawSubject = String(assignment?.subject || "");
  if (taskType !== "DUTY_INVIGILATOR") return normalizeSubject(rawSubject);

  // مراقب الدور لا ينشئ عمودًا مستقلًا.
  // نربطه بأول مادة امتحان في نفس اليوم/الفترة حتى يظهر داخل عمود الامتحان نفسه.
  const daySubjects = Array.isArray(assignment?.daySubjects)
    ? assignment.daySubjects.map((x: any) => String(x || "").trim()).filter(Boolean)
    : rawSubject.split(/[،,]/).map((x) => String(x || "").trim()).filter(Boolean);

  return normalizeSubject(daySubjects[0] || rawSubject || "مراقب دور");
}
...
export function useResultsDataModel({
  ...
  const dateToSubCols = useMemo(() => {
    ...
    if (examsFromStorage.length) {
      // الأعمدة تُبنى من جدول الامتحانات فقط.
      // مراقب الدور يظهر داخل عمود الامتحان المرتبط به ولا ينشئ عمودًا مستقلًا.
  ...
  const matrix2 = useMemo(() => {
    ...
    for (const a of assignments as any[]) {
      ...
      for (const period of periods) {
        const fallbackExamKey = firstExamSubColKeyByDatePeriod.get(`${dateISO}__${period}`);
        const key = String(a?.taskType || "") === "DUTY_INVIGILATOR" && fallbackExamKey
          ? fallbackExamKey
          : `${dateISO}__${period}__${subject}`;
    ...
    return m;
  }, [assignments, allTeachers, normalizeSubject, firstExamSubColKeyByDatePeriod]);
  ...
  const totalsDetailBySubCol = useMemo(() => {
    ...
    for (const a of assignments as any[]) {
      ...
      const rawKey = colKeyOf(a);
      const period = periodToAMPM(String(a?.period || "AM"));
      const fallbackExamKey = firstExamSubColKeyByDatePeriod.get(`${String(a?.dateISO || "").trim()}__${period}`);
      const key = t === "DUTY_INVIGILATOR" && fallbackExamKey ? fallbackExamKey : rawKey;
      if (!out[key]) {
    ...
    return out;
  }, [assignments, allSubCols, committeesCountBySubCol, examKeyToCommittees, firstExamSubColKeyByDatePeriod]);
  ...
  const teacherTotals = useMemo(() => {
    ...
    for (const a of assignments as any[]) {
      ...
      if (tt === "INVIGILATION" || tt === "RESERVE" || tt === "DUTY_INVIGILATOR") out[teacher] = (out[teacher] || 0) + 1;
    }
    return out;
