...
function normalizeEditableTaskType(taskType: string) {
  const raw = String(taskType || "").trim().toUpperCase();
  if (raw === "REVIEW_FREE" || raw === "CORRECTION_FREE") return "DUTY_INVIGILATOR";
  if (raw.includes("مراجعة") || raw.includes("تصحيح") || raw.includes("مراقب دور") || raw.includes("مراقب الدور")) {
...
export function useResultsTableActions({
  ...
  const addTaskToEmptyCell = (dstTeacher: string, dstColKey: string, taskType: string) => {
    addTaskToResultsEmptyCell({
      ...
      dstTeacher,
      dstColKey,
      taskType: normalizeEditableTaskType(taskType),
      normalizeSubject,
      getUnavailabilityReasonForCell,
      markCellBlocked,
      teacherNameToId,
