...
} from "./resultsCellMutationHelpers";
...
export function addTaskToResultsEmptyCell({
  ...
  const currentAssignments = Array.isArray(run?.assignments)
    ? (run.assignments as any[])
    : [];

  const dstHasAnything =
    getAssignmentsInCell(currentAssignments, tName, colKey, normalizeSubject).length > 0;
  ...
  const missingSlotResult = computeMissingCommitteeSlotForResultsCell({
    taskType: tt,
    currentAssignments,
    dstColKey,
    col,
  ...
  const missingSlotAssignment = missingSlotResult?.assignment || {};

  const committeeNo =
    missingSlotResult?.committeeNo ??
    missingSlotAssignment?.committeeNo ??
    missingSlotAssignment?.committee ??
    missingSlotAssignment?.roomNo ??
    missingSlotAssignment?.room ??
    undefined;

  const invigilatorIndex =
    missingSlotResult?.invigilatorIndex ??
    missingSlotAssignment?.invigilatorIndex ??
    missingSlotAssignment?.slotIndex ??
    missingSlotAssignment?.index ??
    undefined;
