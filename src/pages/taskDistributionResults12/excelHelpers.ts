...
export function mapTaskTypeFromText(v: any): TaskType | string {
  ...
  if (k.includes("مراقب دور") || k.includes("مراقب الدور") || k.includes("مراجعة") || k.includes("مراجعه")) return "DUTY_INVIGILATOR";
  if (k.includes("تصحيح")) return "DUTY_INVIGILATOR";
...
export function parseMatrixCellToAssignments(cellText: string, teacherName: string, dateISO: string): Assignment[] {
  ...
  for (const chunk0 of chunks.length ? chunks : [raw]) {
    ...
    cleaned = cleaned.replace(/مراقبة/g, "").replace(/احتياط/g, "").replace(/مراقب دور/g, "").replace(/مراقب الدور/g, "").replace(/مراجعة/g, "").replace(/تصحيح/g, "");
