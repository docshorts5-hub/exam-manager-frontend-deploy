...
function buildHeader(lang: Lang): OfficialHeader {
  ...
  const governorate = firstText(payload.governorate, payload.regionAr, payload.region, payload.governorateName, payload.directorateGovernorate);
  ...
  return {
    ...
    ministry: firstText(payload.ministry, payload.ministryName, lang === "ar" ? "وزارة التربية والتعليم" : "Ministry of Education"),
    directorate: normalizeDirectorate(firstText(payload.directorateName, payload.educationDirectorate, payload.generalDirectorate, payload.directorate), governorate),
    ...
    semester: firstText(payload.semester, payload.termName, payload.term, ""),
    logoUrl: logo,
  };
}
...
export default function CandidateWrittenWarning12() {
  ...
  const fillCenterData = () => {
    ...
    setForm((prev) => ({
      ...
      date: prev.date || now.toISOString().slice(0, 10),
      day: prev.day || new Intl.DateTimeFormat("ar", { weekday: "long" }).format(now),
      warningHour: prev.warningHour || String(now.getHours()).padStart(2, "0"),
      warningMinute: prev.warningMinute || String(now.getMinutes()).padStart(2, "0"),
    }));
  };
  ...
  const saveToCloud = async () => {
    ...
    try {
      ...
      const ref = await addDoc(collection(db, "tenants", tenantId, "candidateWrittenWarnings12"), payload);
      const row: SavedWarningRow = {
        id: ref.id,
        savedAtISO: formatDateTime(new Date()),
        candidateName: form.candidateName || "-",
        seatNo: form.seatNo || "-",
        subject: form.subject || "-",
        centerName: form.centerName || header.centerName || "-",
        status,
      };
      setSavedRows((prev) => [row, ...prev].slice(0, 12));
    ...
    <div style={pageShellStyle}>
      ...
      <div className="no-print" style={actionBarStyle}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <button type="button" onClick={fillCenterData} style={buttonStyle("linear-gradient(180deg, #dcfce7, #bbf7d0)")}>تعبئة التاريخ والمركز</button>
          <button type="button" onClick={saveToCloud} disabled={saving} style={buttonStyle("linear-gradient(180deg, #dbeafe, #bfdbfe)")}>{saving ? "جاري الحفظ..." : "حفظ الإنذار في السحابة"}</button>
        ...
        <section className="a4-page warning-page">
          ...
          <header className="official-warning-header">
            <div className="logo-side">
              ...
              <div>{header.governorate ? `بمحافظة ${header.governorate}` : "بمحافظة ................"}</div>
            </div>
      ...
      <section className="no-print saved-table-card">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div>
            ...
            <p style={{ margin: "6px 0 0", color: MUTED, fontWeight: 800 }}>يعرض آخر الإنذارات التي تم حفظها من هذه الصفحة. لا يظهر هذا الجدول في الطباعة.</p>
          </div>
          ...
          <table className="saved-table">
            ...
            <tbody>
              {savedRows.length ? (
                ...
                <tr>
                  <td colSpan={7}>لم يتم حفظ أي إنذار من هذه الجلسة بعد.</td>
                </tr>
              )}
            </tbody>
          </table>
