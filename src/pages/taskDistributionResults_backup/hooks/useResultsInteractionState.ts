...
import { buildUnavailabilityIndex, buildUnavailabilityReasonMap, UNAVAIL_UPDATED_EVENT } from "../../../utils/taskDistributionUnavailability";
...
export function useResultsInteractionState() {
  ...
  useEffect(() => {
    const refreshUnavail = () => {
      try {
        setUnavailRules(safeLoadResultsUnavailability());
      } catch {
        setUnavailRules([]);
      }
    };
